...
- 💻 웹과 모바일 앱의 전체 개발 흐름 경험
- ⚙️ Java와 Spring Boot를 활용한 서버 기능 구현
- 🔗 MyBatis를 활용한 데이터베이스 연동 및 SQL 작성
- 🧩 결제·환불, 사용자 알림 및 관리자 운영 기능 구현
- 🗂️ 개인 프로젝트의 DB 구조 및 테이블 관계 설계
- 📊 관리자 통계 대시보드 일부 구현
...
- 클라이언트 요청값을 그대로 저장하지 않고 PortOne 결제 내역과 서버의 상품 정보를 비교했습니다.
- 관리자 처리 결과가 회원 및 콘텐츠 상태에 올바르게 반영되도록 기능 흐름을 구성했습니다.
